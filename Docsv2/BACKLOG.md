...
- PERF-001: RAF Idle Detection -> Too complex, risk/benefit ratio poor
...
### Code Locations:
- `isFormZone()` helper function
- `detectCursorMode()` calls `resolveVisibility()` which uses `isFormZone()`
- `mouseover` handler uses `isFormZone()`
- `mouseout` handler uses `isFormZone()`
...
### Code Locations:
- `resolveVisibility()` in custom-cursor.js
- `mouseover` handler
- `mouseout` handler
