...
|----------|--------|----------|
| `calcPulseScale(time, amplitude)` | Pulse | custom-cursor.js |
| `calcShakeOffset(time, amplitude)` | Shake | custom-cursor.js |
| `calcBuzzRotation(time, amplitude)` | Buzz | custom-cursor.js |
| `calcWobbleMatrix(wState, dx, dy)` | Wobble | custom-cursor.js |
| `resolveEffect(cursorEffect, globalWobble)` | All | custom-cursor.js |

### v5.7 Resolver Integration

As of WP-021, `resolveEffectForElement(el)` determines which effect applies to an element by walking up the DOM (respecting widget boundaries via `findWithBoundary()`). The pure effect functions above are then called in the RAF render loop with the resolved effect.
...
```
+                         +
|  matrixA  matrixC   0   |     +  1 + s*cos(2t)    s*sin(2t)     0  +
|  matrixB  matrixD   0   |  =  |    s*sin(2t)    1 - s*cos(2t)   0  |
|  0        0         1   |     +       0              0          1  +
+                         +

Where:
- s = wobbleScale * 0.5 (stretch factor, 0 to 0.6)
- t = wobbleAngle (direction of movement in radians)
- 2t = double angle for symmetric stretch along movement axis
...
**Note:** Period = 2*pi / 0.05 = ~125.66 frames. At 60fps, this equals ~2.1 seconds per full cycle.
...
| Wave phase | 0-6.28 (~2*pi) | 0-6.28 | Active oscillation |
...
| Time increment | 0.08/frame | 0.08/frame | Per RAF frame |
| Cycle length | 10 units | 10 units | Full cycle (~2.08s @ 60fps) |
| Rotate phase | 0-6.28 (~2*pi) | 0-6.28 | Active oscillation |
| Pause phase | 6.28-10 | 6.28-10 | Ease out to center |
| Amplitude | +/-15deg | +/-12deg | Rotation range |
...
- **Effect + Core Cursor:** Wobble + Classic (ring and dot both stretch), Pulse + Dot (dot pulses alone), Shake + Blend (shaking blended cursor)
- **Effect + Special Cursor:** Wobble + Image (`data-cursor-image-effect="wobble"`), Pulse + Icon (via `data-cursor-icon` + effect), Shake + Text (text cursor shakes)
- **Effect + Hover States:** Wobble persists during hover scale, Effect + Color applies to colored cursor
...
Effects are configured in Elementor Site Settings > Custom Cursor section (Kit postmeta).
See `SOURCE-OF-TRUTH.md` Matrix A for current control IDs.
...
- `SOURCE-OF-TRUTH.md` — Settings matrices and scenarios
- `REF-SETTINGS.md` — Legacy settings reference
- `REF-EDITOR.md` — Editor sync protocol

---

*Last Updated: February 6, 2026 | Version: 5.6 | Migrated to Docsv2*
